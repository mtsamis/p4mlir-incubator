// RUN: p4mlir-opt  --p4hir-inline-subparsers --p4hir-simplify-parsers --canonicalize %s | FileCheck %s
...
module {
  ...
  p4hir.parser @caller(%arg0: !b8i, %arg1: !p4hir.ref<!b8i>)() {
    ...
    // CHECK:  %ipv4_out_arg = p4hir.variable ["ipv4_out_arg"] : <!b8i>
    // CHECK:  %promoted_local = p4hir.variable ["promoted_local"] : <!b8i>

    // CHECK-LABEL: p4hir.state @start
    // CHECK:      p4hir.transition_select %{{.*}} : !b8i {
