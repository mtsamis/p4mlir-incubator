...
module {
  ...
  p4hir.func @f1(%arg0: !p4hir.ref<!b8i> {p4hir.dir = #p4hir<dir inout>, p4hir.param_name = "a"}) {
    ...
    p4hir.scope {
      ...
      p4hir.for : cond {
        ...
        %lt = p4hir.cmp(lt, %val_0, %val_1) : !b8i, !p4hir.bool
        p4hir.condition %lt
      } body {
        %val_0 = p4hir.read %i : <!b8i>
        %eq = p4hir.cmp(eq, %val_0, %c10_b8i) : !b8i, !p4hir.bool
        p4hir.if %eq {
          p4hir.soft_return
        }
        p4hir.yield
    ...
    //            ...
    // CHECK:     %[[ORIG_COND:.*]] = p4hir.cmp(lt, %val_0, %val_1) : !b8i, !p4hir.bool
  ...
  p4hir.func @f2(%arg0: !p4hir.ref<!b8i> {p4hir.dir = #p4hir<dir inout>, p4hir.param_name = "a"}) {
    ...
    p4hir.scope {
      ...
      p4hir.foreach %arg1 : !b8i in %range : !p4hir.set<!b8i> {
        %eq = p4hir.cmp(eq, %arg1, %c10_b8i) : !b8i, !p4hir.bool
        p4hir.if %eq {
          p4hir.soft_return
        }
        p4hir.yield
  ...
  p4hir.func @f3(%arg0: !p4hir.ref<!b8i> {p4hir.dir = #p4hir<dir inout>, p4hir.param_name = "a"}) {
      ...
      p4hir.for : cond {
        ...
        p4hir.scope {
          ...
          p4hir.for : cond {
            ...
            %eq = p4hir.cmp(eq, %add_2, %c100_b8i) : !b8i, !p4hir.bool
            p4hir.if %eq {
              p4hir.soft_return
            }
            p4hir.yield
  ...
  p4hir.func @f4(%arg0: !p4hir.ref<!b8i> {p4hir.dir = #p4hir<dir inout>, p4hir.param_name = "a"}, %arg1: !p4hir.ref<!b8i> {p4hir.dir = #p4hir<dir inout>, p4hir.param_name = "b"}) {
    ...
    p4hir.scope {
      ...
      p4hir.for : cond {
        ...
        %eq = p4hir.cmp(eq, %mod, %c8_b8i) : !b8i, !p4hir.bool
        p4hir.if %eq {
          p4hir.soft_break
        } else {
          %val_5 = p4hir.read %i : <!b8i>
          %mod_6 = p4hir.binop(mod, %val_5, %c10_b8i) : !b8i
          %eq_7 = p4hir.cmp(eq, %mod_6, %c9_b8i) : !b8i, !p4hir.bool
          p4hir.if %eq_7 {
            p4hir.soft_return
          }
        }
    ...
    // CHECK: body {
    // CHECK:   %[[EQ_1:.*]] = p4hir.cmp(eq, %{{.*}}, %c8_b8i) : !b8i, !p4hir.bool
    // CHECK:   p4hir.if %[[EQ_1]] {
    // CHECK:     p4hir.assign %{{.*}}, %loop_break_guard : <!p4hir.bool>
    // CHECK:   } else {
    // CHECK:     %[[EQ_2:.*]] = p4hir.cmp(eq, %{{.*}}, %c9_b8i) : !b8i, !p4hir.bool
    // CHECK:     p4hir.if %[[EQ_2]] {
    // CHECK:       p4hir.assign %{{.*}}, %loop_break_guard : <!p4hir.bool>
