...
OpFoldResult P4HIR::CastOp::fold(FoldAdaptor) {
    ...
    if (auto inputConst = mlir::dyn_cast_if_present<ConstOp>(getOperand().getDefiningOp())) {
        auto srcAttr = inputConst.getValueAs<mlir::TypedAttr>();
        if (auto castResult = P4HIR::foldConstantCast(getType(), srcAttr)) return castResult;
    }
...
OpFoldResult P4HIR::ConcatOp::fold(FoldAdaptor adaptor) {
    if (adaptor.getLhs() && adaptor.getRhs()) {
        auto lhs = mlir::cast<P4HIR::IntAttr>(adaptor.getLhs()).getValue();
        auto rhs = mlir::cast<P4HIR::IntAttr>(adaptor.getRhs()).getValue();
