...
LogicalResult P4HIR::IfOp::canonicalize(P4HIR::IfOp op, PatternRewriter &rewriter) {
    ...
    // NOTE: In the following canonicalizations we directly edit the regions/blocks of the
    // IfOp with modifyOpInPlace. This will not fire notifyBlockErased/notifyBlockInserted
    // events in the case that PatternRewriter has a listener.
    // If this becomes an issue the following code needs to be rewritten with rewriter patterns
    // like clone... + inlineRegion... See RemoveEmptyElseBranch from SCF IfOp for an example.
