    ...
    mlir::Value castKeysetValue(mlir::PatternRewriter &rewriter, mlir::Type newType,
        ...
        if (auto setOp = keyset.getDefiningOp<P4HIR::SetOp>()) {
            ...
            for (auto member : setAttr.getMembers()) {
                auto newAttr =
                    P4HIR::foldConstantCast(newType, mlir::cast<mlir::TypedAttr>(member));
    ...
    std::tuple<mlir::Value, mlir::Value, bool> createMaskOp(mlir::PatternRewriter &rewriter,
        ...
        if (auto constOp = yieldArg.getDefiningOp<P4HIR::ConstOp>()) {
            ...
            if (P4HIR::isUniversalSetValue(yieldArg)) {
                val = mask = llvm::APInt::getZero(destType.getWidth());
                needsMask = true;
            } else {
                auto setAttr = constOp.getValueAs<P4HIR::SetAttr>();

                if (setAttr.getKind() == P4HIR::SetKind::Constant) {
                    val = setAttr.getConstantValue<P4HIR::IntAttr>();
                    mask = llvm::APInt::getAllOnes(val.getBitWidth());
                } else {
                    std::tie(val, mask) = setAttr.getMaskValues();
                    needsMask = true;
                }
            }

            keyVal = rewriter.create<P4HIR::ConstOp>(yieldArg.getLoc(),
                                                     P4HIR::IntAttr::get(destType, val));
            keyMask = rewriter.create<P4HIR::ConstOp>(yieldArg.getLoc(),
                                                      P4HIR::IntAttr::get(destType, mask));
        } else if (auto maskOp = yieldArg.getDefiningOp<P4HIR::MaskOp>()) {
            keyVal = maskOp.getLhs();
            keyMask = maskOp.getRhs();
            needsMask = true;
        } else if (auto setOp = yieldArg.getDefiningOp<P4HIR::SetOp>()) {
            auto mask = llvm::APInt::getAllOnes(destType.getWidth());
            keyVal = setOp.getInput()[0];
            keyMask = rewriter.create<P4HIR::ConstOp>(yieldArg.getLoc(),
                                                      P4HIR::IntAttr::get(destType, mask));
        } else {
