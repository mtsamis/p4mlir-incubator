...
struct InlineParsersPass : public P4::P4MLIR::impl::InlineParsersBase<InlineParsersPass> {
 ...
 private:
    ...
    static void updateNamesAndRefs(P4HIR::ParserOp callerParser, llvm::StringRef prefix,
        ...
        // Rewrite SymbolRefAttrs that have as a root the inlined parser.
        for (auto namedAttr : op->getAttrs()) {
            if (auto symbolAttr = mlir::dyn_cast<mlir::SymbolRefAttr>(namedAttr.getValue())) {
                auto root = symbolAttr.getRootReference();
                auto mod = callerParser->getParentOfType<mlir::ModuleOp>();
                if (!mod.lookupSymbol<P4HIR::ParserOp>(root)) continue;
                ...
                for (auto flatRefAttr : symbolAttr.getNestedReferences()) {
                    if (first)
                        newNestedRefs.push_back(mlir::FlatSymbolRefAttr::get(
                            updateName(flatRefAttr.getAttr(), prefix)));
                    else
                ...
                auto newSymbol =
                    mlir::SymbolRefAttr::get(callerParser.getSymNameAttr(), newNestedRefs);
