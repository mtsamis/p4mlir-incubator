...
P4HIR::ParserStateOp IRUtils::createSubState(mlir::RewriterBase &rewriter,
                                             P4HIR::ParserStateOp state, llvm::Twine suffix) {
    auto parser = state->getParentOfType<P4HIR::ParserOp>();
    auto getUniqueName = [&](llvm::Twine basename) {
        unsigned counter = 0;
...
P4HIR::ParserStateOp IRUtils::SplitStateRewriter::createSubState(llvm::Twine suffix,
