...
class SplitStateRewriter {
 public:
    ...
    P4HIR::ParserStateOp createJoinSubState(llvm::Twine suffix, mlir::Block *ops = nullptr) {
...
class TransitionSelectBuilder {
 public:
    ...
    // Add new cases that transition to `transitionTo` with a keyset equivalent to `caseOp`.
    void addCases(P4HIR::CaseOp caseOp, P4HIR::ParserStateOp transitionTo);
